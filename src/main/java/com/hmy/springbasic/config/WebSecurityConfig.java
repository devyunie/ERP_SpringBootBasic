package com.hmy.springbasic.config;
...
public class WebSecurityConfig {
    ...
    // Class::method
    // - 람다식을 일부로 메서드 참조라고 부른다: 특정 클래스의 메서드를 참조할 때 사용
    ...
    protected SecurityFilterChain configure(HttpSecurity security) throws Exception {
        ...
        security
                // Basic 인증방식에 대한 설정
                // Basic 인증방식을 미사용으로 지정한것
                .httpBasic(HttpBasicConfigurer::disable)
                // Session :
                // 웹애플리케이션에서 클라이언트에 대한 정보를 유지하기 위한 기술
                ...
                // - 모든 클라이언트가 접근 할 수 있도록 허용
                // - 인증된 모든 클라이언트가 접근할 수 있도록 허용
                // - 인증된 클라리언트 중 특정 권한을 가진 클라이언트만 접근할 수 있도록 허용
                .authorizeHttpRequests(request -> request
                    //requestMatchers(): URL패펀, HTTP메서드, + URL패턴, HTTP메서드 마다 접근 허용 방식을 지정하는 메서드
...
class FailedAuthenticationEntryPoint implements AuthenticationEntryPoint{
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response,
            AuthenticationException authException) throws IOException, ServletException {
                ...
                response.getWriter().write("{\"message\": \"인증 및 인가에 실패 했습니다\"}");
    }
